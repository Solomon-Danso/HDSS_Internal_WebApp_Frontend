    ...
    const handlesubmit = async () => {
       ...
       const senderId  = userInfo.UserId;
        ...
        try {
          ...
          formData.append("DepartureFare", DepartureFare);
          ...
          if(!DepartureFare){
            Show.Attention("Departure Fare cannot be null")
            return
          }
         ...
         Show.showLoading("Processing Data")
          ...
          const response = await fetch(apiServer + "AddStudentToDepartureList", {
            method: "POST",
            body: formData,
          });
  ...
  useEffect(()=>{
    ...
    const LoadStudent = async () => {
      ...
      formData.append("SenderId", userInfo.UserId);
          ...
          const response = await fetch(apiServer+"StudentToDepartureList", {
            method: "POST",
            body: formData
          });
...
{theStudents.length > 0 &&
    theStudents.map((data,index) => (
      <DepartureCard data={data} key={index}  />
    ))}
