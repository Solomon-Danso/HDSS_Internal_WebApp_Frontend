...
const [d,sd] = useState("")
      ...
      const deleteDetails = async (event) => {
        ...
        const URL = `api/LMS/DeleteannoucementForStudent?TheId=${data.theId}&SID=${userInfo.staffID}`
        try {
    ...
    const URL = `api/LMS/UpdateannoucementForStudent?TheId=${data.theId}&SID=${userInfo.staffID}`
        try {
          const response = await fetch(apiServer + URL, {
            method: "POST",
            headers: {
