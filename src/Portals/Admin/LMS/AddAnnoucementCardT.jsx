...
const [d,sd] = useState("")
      ...
      const deleteDetails = async (event) => {
        ...
        const URL = `api/LMS/DeleteannoucementForTeacher?TheId=${data.theId}&SID=${userInfo.staffID}`
        try {
    ...
    const URL = `api/LMS/UpdateannoucementForTeacher?TheId=${data.theId}&SID=${userInfo.staffID}`
        try {
          const response = await fetch(apiServer + URL, {
            method: "POST",
            headers: {
