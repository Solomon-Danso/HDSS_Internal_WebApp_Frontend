...
import  {ViewPicturesCard}  from './ViewPicturesCard'

import {  SearchPictures, SearchSubject, SubTeacher,ViewClasses,ViewStudents, ViewTeachers, apiServer } from '../../../Constants /Endpoints'
...
const StudentInfo = () => {
    ...
    useEffect(() => {
        ...
        const fetchSearchResults = async () => {
          ...
          const URL = `api/Admin/AdminSearchPictures?searchTerm=${searchTerm}`
          try {
 ...
 const studentDetails = async () => {
  ...
  const URL = `api/AllGetters/viewPictures?Sub=${d}&Level=${c}`;
...
{searchResult && (
          <StudentListResult>
            {studentList.length > 0 &&
              studentList.map((data, index) => <ViewPicturesCard data={data} key={index} index={index}/>)}
          </StudentListResult>
        )}
...
{subj.length > 0 &&
    subj.map((data,index) => (
      <ViewPicturesCard data={data} key={index} index={index} />
    ))}
