...
const StudentInfo = () => {
    ...
    useEffect(() => {
        ...
        const fetchSearchResults = async () => {
          ...
          const URL = `api/Admin/SearchVideo?searchTerm=${searchTerm}&StaffID=${userInfo.staffID}`
          try {
      ...
      useEffect(() => {
        if(userInfo.staffID){
          const URL=`api/LMS/ViewAllVideoTeachers?ID=${userInfo.staffID}`
...
useEffect(() => {
 ...
 const [d,sd] = useState("")
      ...
      const studentDetails = async (event) => {
       ...
       Show.showLoading("Processing Data");
    const URL=`api/LMS/UploadVideo?ID=${userInfo.staffID}`
        ...
        try {
          ...
          if (response.ok) {
           Show.hideLoading();
           Show.Success("Video Uploaded Successfully")
...

{
   ...
   <AboutHeader2
     ...
     >
       {dropper?"Minimize":"Upload A Video"}
     </AboutHeader2> <br/>
...
<FeesRow>
<FeesIcons>
<RiVideoUploadLine color={colors.icon}/>
</FeesIcons>
...
{searchResult && (
          <StudentListResult>
            {studentList.length > 0 &&
              studentList.map((data, index) => <UploadVideoCard data={data} key={index} index={index}/>)}
          </StudentListResult>
        )}
...
{subj.length > 0 &&
    subj.map((data,index) => (
      <UploadVideoCard data={data} key={index} index={index} />
    ))}
