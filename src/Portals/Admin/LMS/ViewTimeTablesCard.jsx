...
export const ViewTimeTablesCard = ({ data,index }) => {
  ...
  useEffect(() => {
    ...
    setspecificRole(spRole);
      ...
      const deleteDetails = async (event) => {
        Show.showLoading("Removing Timetable ")
        const URL=`api/StudentApp/deleteTimeTables?Id=${data.id}&SID=${userInfo.staffID}`
        ...
        try {
          ...
          if (response.ok) {
            ...
            Show.hideLoading()
            Show.Success("Timetable removed successfully");
